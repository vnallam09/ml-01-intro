"""app_venkat_teja.py - Phase 4 technical modification.

A copy of the working example (app_case.py) with one small,
observable, explainable change:

    MODIFICATION: The model evaluation now also reports the
    Root Mean Squared Error (RMSE) alongside MAE and R-squared.
    RMSE is in the same units as the target (score points) and
    penalizes large prediction errors more heavily than MAE,
    so comparing RMSE to MAE hints at whether a few predictions
    miss by a lot (RMSE >> MAE) or errors are uniform (RMSE ~ MAE).
...
Process:
    ...
    - Evaluate model performance (MAE, RMSE, R-squared).
...
def train_model(df_clean: pd.DataFrame) -> LinearRegression:
    ...
    # MODIFICATION (Phase 4): also report RMSE.
    # RMSE squares each error before averaging, so it punishes big misses.
